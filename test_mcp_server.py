...
class TestMetadataWarningsConsistency:
    ...
    def test_no_metadata_warnings_when_complete(self):
        from mcp_server import _build_metadata_warnings
        w = _build_metadata_warnings({"title": "T", "channel": "C", "published": "2026-01-01"},
                                     {"title": "oembed", "channel": "oembed", "published": "pytubefix"})
        assert len(w) == 0
    ...
    def test_total_metadata_failure_warning(self):
        ...
        assert len(w) == 1
        assert "All metadata" in w[0]["message"]


class TestCaptionType:

    def test_caption_type_manual(self):
        assert True

    def test_caption_type_auto_generated(self):
        assert True
...
class TestDeepCacheValidation:
    ...
    def test_meta_title_wrong_type_is_miss(self, cache_dir):
        ...
        with patch("yt_transcript.CACHE_DIR", cache_dir):
            ...
            _cache_path("vid1234567a", ["en"]).write_text(json.dumps(entry))
            assert load_from_cache("vid1234567a", ["en"]) is None
    ...
    def test_cached_at_invalid_format_is_miss(self, cache_dir):
        from yt_transcript import load_from_cache, _cache_path
        with patch("yt_transcript.CACHE_DIR", cache_dir):
            _cache_path("vid1234567a", ["en"]).write_text(
                json.dumps(_make_cache_entry(cached_at="not-a-date")))
            assert load_from_cache("vid1234567a", ["en"]) is None

    def test_requested_languages_not_list_of_str_is_miss(self, cache_dir):
        from yt_transcript import load_from_cache, _cache_path
        with patch("yt_transcript.CACHE_DIR", cache_dir):
            _cache_path("vid1234567a", ["en"]).write_text(
                json.dumps(_make_cache_entry(requested_languages=[1, 2, 3])))
            assert load_from_cache("vid1234567a", ["en"]) is None

    def test_nan_start_in_segment_is_miss(self, cache_dir):
        from yt_transcript import load_from_cache, _cache_path
        with patch("yt_transcript.CACHE_DIR", cache_dir):
            _cache_path("vid1234567a", ["en"]).write_text(json.dumps(
                _make_cache_entry(segments=[
                    {"text": "hi", "start": float("nan"), "duration": 1.0}])))
            assert load_from_cache("vid1234567a", ["en"]) is None

    def test_bool_start_in_segment_is_miss(self, cache_dir):
        from yt_transcript import load_from_cache, _cache_path
        with patch("yt_transcript.CACHE_DIR", cache_dir):
            _cache_path("vid1234567a", ["en"]).write_text(json.dumps(
                _make_cache_entry(segments=[
                    {"text": "hi", "start": True, "duration": 1.0}])))
            assert load_from_cache("vid1234567a", ["en"]) is None


class TestBareHostname:

    def test_bare_youtube_hostname_accepted(self):
        from yt_transcript import extract_video_id
...
class TestFinalValidationCloseout:

    def test_meta_sources_bogus_value_is_miss(self, cache_dir):
        from yt_transcript import load_from_cache, _cache_path
        with patch("yt_transcript.CACHE_DIR", cache_dir):
            entry = _make_cache_entry()
            entry["meta_sources"]["title"] = "bogus"
            _cache_path("vid1234567a", ["en"]).write_text(json.dumps(entry))
            assert load_from_cache("vid1234567a", ["en"]) is None

    def test_cached_at_impossible_date_is_miss(self, cache_dir):
        from yt_transcript import load_from_cache, _cache_path
        with patch("yt_transcript.CACHE_DIR", cache_dir):
            _cache_path("vid1234567a", ["en"]).write_text(
                json.dumps(_make_cache_entry(cached_at="2026-99-99")))
            assert load_from_cache("vid1234567a", ["en"]) is None

    def test_infinity_start_plus_duration_is_miss(self, cache_dir):
        from yt_transcript import load_from_cache, _cache_path
        with patch("yt_transcript.CACHE_DIR", cache_dir):
            _cache_path("vid1234567a", ["en"]).write_text(json.dumps(
                _make_cache_entry(segments=[
                    {"text": "hi", "start": 1e308, "duration": 1e308}])))
            assert load_from_cache("vid1234567a", ["en"]) is None

    def test_large_integer_start_no_crash(self, cache_dir):
        from yt_transcript import load_from_cache, _cache_path
        with patch("yt_transcript.CACHE_DIR", cache_dir):
            _cache_path("vid1234567a", ["en"]).write_text(json.dumps(
                _make_cache_entry(segments=[
                    {"text": "hi", "start": 10**400, "duration": 1.0}])))
            assert load_from_cache("vid1234567a", ["en"]) is None

    def test_empty_host_url_rejected(self):
        from yt_transcript import extract_video_id
        with pytest.raises(ValueError):
            extract_video_id("https:///watch?v=dQw4w9WgXcQ")
