...
class FakeTransport:
    ...
    `players` maps client name -> fixture file name, HttpResponse, or Exception.
    ...
    def __call__(self, method, url, headers, body):
        ...
        if method == "POST":
            ...
            return self._resolve(self.players[client], lambda n: load(n).encode("utf-8"))
...
class TestParsing:
    ...
    def test_parity_with_youtube_transcript_api(self):
        """Removed together with the library dependency."""
        from youtube_transcript_api._transcripts import _TranscriptParser
        xml = load("timedtext_ios_en_manual.xml")
        expected = [{"text": s.text, "start": s.start, "duration": s.duration}
                    for s in _TranscriptParser().parse(xml.encode("utf-8"))]
        assert parse_timedtext(xml.encode("utf-8")) == expected
